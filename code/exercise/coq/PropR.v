...
Proof.
  intros. replace (2 * n) with (n + n). apply b_sum with (n:=n) (m:=n).
  apply H. apply H. simpl. replace (n + 0) with n. reflexivity. rewrite plus_0_r.
  reflexivity.
Qed.
...
Proof.
   intros n. induction m. intros H. simpl. apply b_0.
   intros. simpl. apply b_sum. apply H. apply IHm. apply H.
Qed.
(** [] *)
...
Proof.
 intros n m H H'. induction H. induction H'.
 apply g_0.
 apply g_plus3. apply H'.
 apply g_plus5. apply H'.
 remember (n + m) as o. replace (3 + n + m) with (3 + o). apply g_plus3. apply IHgorgeous.
 rewrite Heqo. reflexivity.
 remember (n + m) as o. replace (5 + n + m) with (5 + o). apply g_plus5. apply IHgorgeous.
 rewrite Heqo. reflexivity.
 Qed.
(** [] *)
...
Proof.
 ...
 simpl. assumption.
 remember (n + m) as o. replace (3 + n + m) with (3 + o). apply g_plus3. 
 apply gorgeous__beautiful in IHbeautiful1. apply IHIHbeautiful1. apply IHbeautiful1.
 rewrite Heqo. reflexivity.
 remember (n + m) as o. replace (5 + n + m) with (5 + o). apply g_plus5. 
 apply gorgeous__beautiful in IHbeautiful1. apply IHIHbeautiful1. apply IHbeautiful1.
 rewrite Heqo. reflexivity.
Qed. 
(** [] *)
...
Theorem ev_sum : forall n m,
   ev n -> ev m -> ev (n+m).
Proof. 
  intros. induction H. apply H0. simpl. apply ev_SS. assumption. Qed.
...
Theorem SSSSev__even : forall n,
  ev (S (S (S (S n)))) -> ev n.
Proof.
  intros. inversion H. inversion H1. assumption. Qed.
