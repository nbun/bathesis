...
Proof. intros. destruct p as [n m]. reflexivity. Qed.
(** [] *)
...
Fixpoint oddmembers (l:natlist) : natlist :=
  match l with
    | nil => nil
    | n :: ls =>  if (oddb n) then n :: (oddmembers ls) else (oddmembers ls)
end.
...
Fixpoint countoddmembers (l:natlist) : nat :=
  match l with
    | nil => 0
    | n :: ls => if (oddb n) then (S (countoddmembers ls)) else (countoddmembers ls)
end.
...
Fixpoint count (v:nat) (s:bag) : nat := 
  match s with
    | nil => 0
    | n :: ss => if (beq_nat n v) then S (count v ss) else (count v ss)
end.
...
Definition member (v:nat) (s:bag) : bool := 
  if (negb (beq_nat (count v s) 0)) then true else false.
...
Theorem count_add : forall b:bag, forall n:nat,
  count n (add n b) = S (count n b).
Proof.
  intros. simpl. replace (beq_nat n n) with true. reflexivity.
  induction n as [|n']. reflexivity. simpl. rewrite <- IHn'. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros. induction l as [|l ls].
  reflexivity.
  simpl. rewrite rev_snoc. rewrite IHls. reflexivity.
Qed.
...
Theorem rev_t : forall (l:natlist) (n:nat),
  rev (n :: l) = snoc l n.
Proof.
  intros. induction l as [|l ls].
  reflexivity.
...
Proof.
  intros. induction l1 as [| l ls].
  simpl. rewrite app_nil_end. reflexivity.
  simpl. rewrite snoc_append. rewrite snoc_append. rewrite IHls.
...
Proof.
  intros. induction l1 as [| l ls].
  reflexivity.
  destruct l. simpl. rewrite IHls. reflexivity.
  simpl. rewrite IHls. reflexivity.
Qed.
...
Fixpoint beq_natlist (l1 l2 : natlist) : bool :=
  ...
  | n :: _, nil => false
  | nil, n :: _ => false
  | n :: ns, m :: ms => if (beq_nat n m) then (beq_natlist ns ms) else false
end.
...
Proof.
  intros. induction l as [|l ls].
  reflexivity.
  simpl. rewrite <- beq_nat_refl. rewrite IHls. reflexivity.
Qed.
(** [] *)
...
Theorem cons_snoc_app : forall (l:natlist) (n:nat),
n :: (snoc l n) = [n] ++ l ++ [n].
Proof.
  intros. induction l as [| l ls].
  reflexivity.
...
Proof. intros. simpl. rewrite <- beq_nat_refl. reflexivity. Qed.
(** [] *)
