...
Destruct consideres every single case of a variable, which is useful if 
you have a boolean value or anything with a small state space.
A natural number has infinite states and therefore can't be simplified
using this tactic. Induction consideres only two cases: n = 0 and n = n' + 1,
with the assumption, that the proposition holds for n. 
...
Proof.
  ...
  replace (n + m) with (m + n). reflexivity. rewrite plus_comm. reflexivity.
Qed.
(** [] *)
