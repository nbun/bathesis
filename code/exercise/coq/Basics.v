...
Fixpoint bin_to_nat (b: bin) : nat :=
  match b with
    | Z => 0
    | D n => 2 * bin_to_nat(n)
    | DI n => 1 + 2 * bin_to_nat(n)
  end.

Example test_bin_incr1 : 0 + 1 = bin_to_nat (DI Z).
Proof. reflexivity. Qed.
Example test_bin_incr2 : bin_to_nat (D(D(D(DI(Z))))) = 8.
Proof. reflexivity. Qed.
Example test_bin_incr3 : bin_to_nat (incr(D(DI(Z)))) = bin_to_nat (D(DI(Z))) + 1.
Proof. reflexivity. Qed.
