...
Fixpoint split
  ...
  match l with
  | [] => (nil, nil)
  | (x,y) :: xys => let (left, right) := split xys in (x::left, y::right)
end.
...
Theorem snoc_w : forall (X Y : Type) (f: X -> Y) (l : X) (ls : list X),
map f (snoc ls l) = snoc (map f ls) (f l).
Proof.
  intros. induction ls.
  reflexivity. 
  assert (snoc (x :: ls) l = x :: (snoc ls l)). reflexivity.
  rewrite H.
  assert (map f (x :: snoc ls l) = (f x) :: (map f (snoc ls l))). 
  reflexivity.
...
Proof.
  ...
  reflexivity.
  simpl. rewrite <- IHls. rewrite snoc_w. reflexivity.
Qed.
(** [] *)
