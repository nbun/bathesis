...
curroq (c : r) cr crs cb = case c of
                                ...
                                '\''-> case (r!!0) of
                                            '\\' -> ['"', '\\', replaceBadChar (r!!1), '"'] ++ (curroq (drop 3 r) cr crs cb)
                                            otherwise -> ['"' , replaceBadChar (r!!0), '"'] ++ (curroq (drop 2 r) cr crs cb)
...
replaceBadChar :: Char -> Char
replaceBadChar x = case x of
                        '"' -> '\''
                        otherwise -> x
...
textUntilQuote []      = []
textUntilQuote (c : s) | c == '"'  = ['"']
                       | otherwise = c : (textUntilQuote s)
